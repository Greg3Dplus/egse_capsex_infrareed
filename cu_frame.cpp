...
CU_Frame::CU_Frame(void* header_ar, quint8 nb_bytes_per_pixel, quint8 cam_fw_type, quint8 nb_bits)
{
    this->init_CU_Frame(nb_bytes_per_pixel, cam_fw_type);
...
//            this->packet.append(static_cast<qint8>((pix >> 8*(this->nb_bytes_per_pixel-byte_num-1)) & 0xff));
            this->packet[byte_num + pix_idx] = static_cast<qint8>((pix >> 8*(this->nb_bytes_per_pixel-byte_num-1)) & 0xff);
        }
    }
}
...
void CU_Frame::init_CU_Frame(quint8 nb_bytes_per_pixel, quint8 cam_fw_type)
{
    ...
    this->setCMVHeaderSize(HEADER_SIZE*NB_PXL_PER_CC);

}

quint8 CU_Frame::getBinningX() {
    //Get Binning
    quint8 binX;
    binX  = (this->getHeader_field(4) & 0x30) >> 4;
    binX = static_cast<quint8>(pow(2, binX));
    return binX;
}
...
bool CU_Frame::init_pkt(void* ImaqBuffer) {
//    this->packet.clear();
//    this->packet.append(header_ar);
    this->packet = (Int8*)ImaqBuffer;
...
//     for (quint8 i = 0; i < HEADER_SIZE; i++) {
//       qDebug().noquote()  << "header (" + QString::number(i,10) + ") = 0x" + QString::number(this->getHeader_field(i),16);
//     }

//     Check that packet size is correct
    if (this->getPacketSize() != this->compute_ref_PktSize(this->getWidth(), this->getHeight()+1, this->getFooter_size()) - 4*NB_PXL_PER_CC*HEADER_SIZE ) {
        qDebug () << "ERROR: Packet size = " + QString::number(this->getPacketSize(), 10) + ", expected " + QString::number(this->compute_ref_PktSize(this->getWidth(), this->getHeight()+1, this->getFooter_size()) - 4*224, 10);
        emit this->CU_error();
        return false;
    }
    ...
    this->binning_x = this->getBinningX();
//    qDebug () << " BINNING X = " + QString::number(this->binning_x, 10);
    return true;
}

quint8 CU_Frame::getByte(quint32 address) {
    return static_cast<quint8>(this->packet[static_cast<int>(address)]);
}
...
//          qDebug().noquote() << "Byte_val = 0x" + QString::number(Byte_val,16);
         // qDebug().noquote() << "Address  = " + QString::number(this->nb_bytes_per_pixel*address+byte_num,10);
         // qDebug().noquote() << "------------ ";

         data |= static_cast<quint32>(Byte_val << (8 * byte_num));
    }
    // return data >> (8-nb_bits);
...
quint8 CU_Frame::getHeader_field(quint16 num_field) {
    ...
    //qDebug().noquote() << "nb_bytes_per_pixel = " + QString::number(this->nb_bytes_per_pixel,10);
    //qDebug().noquote() << "Address header = " + QString::number(this->nb_bytes_per_pixel*(NB_PXL_PER_CC*num_field+(NB_PXL_PER_CC-1)));

    return static_cast<quint8>(this->getData(NB_PXL_PER_CC*num_field+(NB_PXL_PER_CC-1), 8));
}

// num_field input is given in "pixels", not in Bytes
quint8 CU_Frame::getFooter_field(quint16 num_field) {
    return static_cast<quint8>(this->getData(this->CMV_HEADER_SIZE + this->getWidth() * this->getHeight() + NB_PXL_PER_CC*num_field+(NB_PXL_PER_CC-1), 8));
}
...
//    return this->getData(this->CMV_HEADER_SIZE  + num_pixel, nb_bits);
        return this->getData(this->getWidth()  + num_pixel, nb_bits);  /// HEADER fait une ligne entiere
...
quint32 CU_Frame::getPacketSize(void) {
   ...
   return this->compute_ref_PktSize(this->getWidth(), this->getHeight()+1, this->getFooter_size()) - 4*NB_PXL_PER_CC*HEADER_SIZE;
}
...
//        qDebug().noquote()  << "packet length "+ QString::number(data_num,10)+ " = " + QString::number(data,16);
    }
...
quint16 CU_Frame::getGain(void) {
    return  static_cast<quint16> ((this->getHeader_field(19) << 8) | this->getHeader_field(18));
}
...
//        }
        ...
        double  exp_time_int = double(integration_time)/(74.25*1000);
//        qDebug().noquote() << "exp_time_int computation" + QString::number(quint32(exp_time_int),10);
//       }
//       else  { //// A DECOMtER EN CAS DE XTRIG

//             exp_time_int =  (H_Period*(((this->getHeight()*binning_y)+VB1+frameInfo)-integration_time)) + 14.26; // imx_freq
//        }
        out << "Integration time : "   << dec << exp_time_int  << " ms" << " (register set to 0x" << hex << integration_time << ")\n";
...
void CU_Frame::copyCU_Frame(CU_Frame* input_frame)
{
    ...
    this->packet             = (Int8*)(input_frame->getPacket());
...
QPixmap CU_Frame::toPixMap(quint8 color) {
    ...
    quint16 height = this->getHeight();
    // B&W

//     int t0 = this->time->elapsed();
//     qDebug().noquote() << "#### TIME before Pixmap (ms)" + QString::number(t0,10);
     /***********************************************************/
     // for(quint16 line = 1; line < height+1; line++) {
     //     for(quint16 pix = 0; pix < (width*4); pix+=4) {
     //       qDebug().noquote() << "P "+ QString::number(pix+(line*4)*width,10) + "  " + QString::number(static_cast<Int8>((this->packet[pix+line*4*width+2]<<16) | ((this->packet[pix+line*4*width+1])<<8) | (this->packet[pix+line*4*width+0])) >> (this->getNb_bits() - 8),10);
     //     }
     // }
     /************************************************************/

// gray 5 FPS RGB// no problem on binning

    // for(quint16 line = 1; line < height+1; line++) {

         // for(quint16 pix = 0; pix < (width*4); pix+=4) {

            // this->packet[pix+(line*4)*width] = static_cast<Int8>((this->packet[pix+line*4*width+2]<<16) | ((this->packet[pix+line*4*width+1])<<8) | (this->packet[pix+line*4*width+0])) >> (this->getNb_bits() - 8);
            // this->packet[pix+(line*4)*width +1 ] =  this->packet[pix+(line*4)*width];
            // this->packet[pix+(line*4)*width +2 ] =  this->packet[pix+(line*4)*width];
...
//                this->packet[pix/4+(line)*width] = static_cast<Int8>((this->packet[pix+line*4*width+2]<<16) | ((this->packet[pix+line*4*width+1])<<8) | (this->packet[pix+line*4*width+0])) >> (this->getNb_bits() - 8);
//            }
//        }
...
//    qDebug().noquote() << "#### TIME after Pixmap (ms)" + QString::number(t1-t0,10);


    int step = 0;

    // QImage needs an alignment with 32-bits. We cannot use Format GrayScale 8
//    return QPixmap::fromImage(QImage((uchar*)(this->packet+width), width, height, step, QImage::Format::Format_Grayscale8));

    //gray 5 FPS RGB
    //return QPixmap::fromImage(QImage((uchar*)(this->packet+width*4), width, height, step, QImage::Format::Format_RGB32));
